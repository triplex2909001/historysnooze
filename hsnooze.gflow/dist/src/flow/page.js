...
import { CreditLimitError, GenerationBlockedError, GenerationFailedError, LoginRequiredError, ManualActionRequiredError, RateLimitedError } from "../errors.js";
...
import { confirmPicker, dismissOpenLayers, navigateToProject, pickModel, pickOption, projectIdFromUrl, projectSubUrl } from "./ui.js";
...
export class FlowPage {
    ...
    flowUrl;
    characters;
    constructor(page, flowUrl = FLOW_URL) {
        this.page = page;
    ...
    async runJob(input) {
        ...
        const resolvedChars = input.resolvedCharacters ??
            input.job.resolvedCharacters ??
            [];
    ...
    async resultSrcs(type) {
        if (type === "video") {
            const videoSrcs = await this.page.$$eval("video", (vids) => vids
            ...
            if (videoSrcs.length > 0)
                return videoSrcs;
            return this.page.$$eval("flow-a2ui-video-option:not(:has(flow-soupy-overlay)):not(:has([class*='soupy'])) img, .video-container:not(:has(flow-soupy-overlay)):not(:has([class*='soupy'])) img, flow-video-card:not(:has(flow-soupy-overlay)) img, [class*='video-option']:not(:has(flow-soupy-overlay)) img", (imgs) => imgs
                .map((i) => i.currentSrc || i.src)
                .filter((src) => /flow-content\.google|flow\.google\.com\/asb|media\.getMediaUrlRedirect|blob:|data:/i.test(src)));
        }
        return this.page.$$eval("img", (imgs) => imgs
    ...
    async applySettings(job) {
        ...
        if (await saveBtn.isVisible().catch(() => false)) {
            if (job.ratio) {
                ...
                if (await ratioToggle.count())
                    await ratioToggle.click().catch(() => undefined);
            }
            if (job.model) {
                ...
                if (await picker.count()) {
                    ...
                    if (await modelItem.count())
                        await modelItem.click().catch(() => undefined);
        ...
        if (job.ratio && RATIO_ICON[job.ratio])
            await pickOption(this.page, new RegExp(`^${RATIO_ICON[job.ratio]}`));
        if (job.type === "video" && job.duration)
            await pickOption(this.page, new RegExp(`^${job.duration}s$`));
    ...
    */
    async clearIngredients() {
        const removeButtons = this.page.locator('[data-testid="remove-ingredient"], button[aria-label*="Remove ingredient" i], [data-testid="ingredient-chip"] button, .ingredient-chip button, [data-slot="ingredients"] button[aria-label*="remove" i], .ingredients-container button[aria-label*="remove" i]');
     ...
     */
    async uploadIngredients(ingredientPaths) {
        if (!ingredientPaths || ingredientPaths.length === 0)
            return;
    ...
    async uploadFrame(slot, filePath) {
        const slotKey = slot.toLowerCase();
    ...
    async waitForResults(before, expected, timeoutMs, type) {
        const locators = flowLocators(this.page);
