...
class TestDualOperationParity(unittest.TestCase):
    ...
    def setUp(self):
        root_wf = Path(__file__).resolve().parent.parent / ".github" / "workflows" / "render_parallel.yml"
        sub_wf = Path(__file__).resolve().parent.parent / "hsnooze.render" / ".github" / "workflows" / "render_parallel.yml"
        self.gha_workflow_path = root_wf if root_wf.exists() else sub_wf
