"""
HISTORYSNOOZE: SCRIPT MASTER FACTORY
Module: script_master_factory.py
...
def write_modular_file(filename: str, p1_name: str, p1_list: list, p2_name: str = None, p2_list: list = None):
    ...
    words1 = sum(len(p.split()) for p in p1_list)
    print(f"Written: {filename} -> {p1_name}: {words1} words ({len(p1_list)} paras)", end="")
    if p2_name and p2_list:
        words2 = sum(len(p.split()) for p in p2_list)
        print(f", {p2_name}: {words2} words ({len(p2_list)} paras)", end="")
    print()
